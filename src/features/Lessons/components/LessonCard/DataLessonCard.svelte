<script lang="ts">
	...
	import { type AgeGroups, type Currencies, type SkillLevels } from "$src/lib/server/db/schemas/normalized";
    ...
    let { lessonData, currencies, skillLevels, ageGroups }: Props = $props()
    ...
    <div class="flex flex-row justify-between gap-0.5 px-1 py-4">
        ...
        <div class="fares flex flex-col justify-center items-center w-[35%] border-2 border-border rounded-lg">
            <div class="price-time flex flex-col gap-2 justify-center items-center  ">
                <div class="price flex flex-row items-center justify-center border-b border-border text-[1.4rem]">{lessonData.basePrice} <span class="currency text-textNeutral text-xs">{mappedCurrency}</span></div>
